...
pp = oracledb.PoolParams(user=username, password=password)
dsn = "warehouse"
...
def fetch_subject(subject_id):
    ...
    Returns None if the subject was not found.
    """
...
def process_subject(row):
    ...
    schedule_fall, schedule_iap, schedule_spring = (
        fetch_schedules(subject_id, year))
    if schedule_fall:
...
def fetch_schedules(subject_id, year):
    """
            ...
            with connection.cursor() as cursor:
                cursor.execute("""
                    SELECT meet_place, meet_time, is_lecture_section,
                        is_recitation_section, is_lab_section,
                        is_design_section
                    FROM subject_offered
                    WHERE subject_id = :subject_id
                    AND term_code = :term_code
                    AND is_master_section = 'N'
                    ORDER BY is_lecture_section DESC,
                        is_recitation_section DESC, is_lab_section DESC,
                        section_id
                """, subject_id=subject_id, term_code=term_code)
                rows = cursor.fetchall()

        for meet_place, meet_time, lec, rec, lab, des in rows:
            if lec == "Y":
                dest = lectures
            elif rec == "Y":
                dest = recitations
